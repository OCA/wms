...
class StockPackageLevel(models.Model):
    ...
    allowed_location_dest_ids = fields.Many2many(
        'stock.location', compute='_compute_allowed_location_dest_ids',
        string='Allowed destinations'
    )

    @api.depends(
        'package_id', 'package_id.package_storage_type_id',
        ...
        'package_id.package_storage_type_id.storage_location_sequence_ids.location_id.children_ids',
        # Dependency on quant_ids managed by cache invalidation on create/write
        'picking_id', 'picking_id.location_dest_id',
    ...
    def _compute_allowed_location_dest_ids(self):
        ...
        for pack_level in self:
            ...
            if (
                pack_level.package_id.package_storage_type_id and
                pack_level.picking_type_code != 'outgoing'
            ):
    ...
    def _get_allowed_location_dest_ids(self):
        package_locations = self.env["stock.storage.location.sequence"].search(
            [
                ("package_storage_type_id", "=", self.package_id.package_storage_type_id.id),
