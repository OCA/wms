...
const Reception = {
        ...
        <Screen :screen_info="screen_info">
                ...
                <div class="button-list button-vertical-list full">
                    <v-row align="center">
                        <v-col class="text-center" cols="12">
                            <btn-action @click="state.on_manual_selection">{{ $t("Manual selection") }}</btn-action>
                        </v-col>
                    </v-row>
                </div>
            </template>
            ...
            <template v-if="state_is('select_move')">
                ...
                <div class="button-list button-vertical-list full">
                    <v-row align="center">
                        <v-col class="text-center" cols="12">
                            <btn-action @click="state.on_mark_as_done">{{ $t("Mark as Done") }}</btn-action>
                        </v-col>
                    </v-row>
                </div>
            </template>
            <template v-if="state_is('confirm_over_reception')">
                <div class="button-list button-vertical-list full">
                    <v-row align="center">
                        <v-col class="text-center" cols="12">
                            <btn-action action="todo" @click="state.on_confirm">{{ $t("Confirm") }}</btn-action>
                        </v-col>
                    </v-row>
                    <v-row align="center">
                        <v-col class="text-center" cols="12">
            ...
            <template v-if="state_is('set_lot')">
                ...
                <div class="button-list button-vertical-list full">
                    <v-row align="center">
                        <v-col class="text-center" cols="12">
                            <btn-action @click="state.on_confirm_lot" :disabled="!is_set_lot_possible()">{{ $t("Continue") }}</btn-action>
                        </v-col>
                    </v-row>
                    <v-row align="center">
                        <v-col class="text-center" cols="12">
            ...
            <template v-if="state_is('set_quantity')">
                ...
                <div class="button-list button-vertical-list full">
                    <v-row v-if="show_with_pack_actions" align="center">
                        <v-col class="text-center" cols="12">
                            <btn-action @click="state.on_add_to_existing_pack">{{ $t("Existing pack") }}</btn-action>
                        </v-col>
                    </v-row>
                    <v-row v-if="show_with_pack_actions" align="center">
                        <v-col class="text-center" cols="12">
                            <btn-action @click="state.on_create_new_pack">{{ $t("New pack") }}</btn-action>
                        </v-col>
                    </v-row>
                    <v-row v-if="show_without_pack_actions" align="center">
                        <v-col class="text-center" cols="12">
                            <btn-action @click="state.on_process_without_pack">{{ $t("Process without pack") }}</btn-action>
                        </v-col>
                    </v-row>
                    <div class="button-list button-vertical-list full">
                        <v-row align="center">
    ...
    methods: {
        ...
        picking_display_fields: function () {
            return [
                ...
                {path: "partner.name", label: this.$t("Partner")},
                {path: "carrier.name", label: this.$t("Carrier")},
                {
                    path: "scheduled_date",
                    renderer: (rec, field) => {
                        return (
                            this.$t("Scheduled Date: ") +
        ...
        select_document_display_fields: function () {
            ...
            fields.push({path: "move_line_count", label: this.$t("Lines")});
            return fields;
        },
        manual_select_options_for_select_document: function (today_only = false) {
            return {
                group_title_default: today_only
                    ? this.$t("Receptions to process today")
                    : this.$t("Receptions to process"),
        ...
        line_product_detail_options: function () {
            return {
                ...
                fields: [
                    ...
                    {path: "lot.name", label: this.$t("Lot"), klass: "loud"},
                    {
        ...
        manual_select_options_for_select_dest_package: function () {
            return {
                group_title_default: this.$t("Packs available"),
                group_color: this.utils.colors.color_for("screen_step_todo"),
                ...
                list_item_options: {
                    fields: [
                        ...
                        },
                        {
                            ...
                            label: this.$t("Lines already in pack"),
                            display_no_value: true,
                        },
                        {path: "storage_type.name", label: this.$t("Package type")},
                    ],
                },
            };
        },
    ...
    data: function () {
        return {
            ...
            filter_input_placeholder: this.$t("Find an operation"),
            filtered_pickings: [],
        };
    },
};
