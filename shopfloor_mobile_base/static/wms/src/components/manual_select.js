    ...
    methods: {
        ...
        */
        ...
        selected_color_klass(rec, modifier) {
            let color = this.utils.colors.color_for("item_selected");
            if (rec.qty_done && rec.qty_done < rec.quantity)
                color = this.utils.colors.color_for("item_selected_partial");
            if (rec.qty_done && rec.qty_done > rec.quantity)
                color = this.utils.colors.color_for("item_selected_excess");
            return "active " + color + (modifier ? " " + modifier : "");
        },
    },
    computed: {
        has_records() {
                        ...
                        <v-list-item-action>
                            <div class="action action-select">
                                <v-btn icon x-large rounded>
                                    <input
                                        :class="['sf-checkbox', is_selected(rec) ? selected_color_klass(rec, 'darken-3') : '']"
                                        type="checkbox"
                                        :input-value="rec.id"
                                        :true-value="rec.id"
                                        :value="rec.id"
